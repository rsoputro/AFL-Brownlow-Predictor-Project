...
df2 = pivot_longer(df1, names_to="stat", values_to="value", cols=stat_cols)
...
df <- df1
# Rename columns
names(df)[names(df) == 'Group.1'] <- 'playerId'
names(df)[names(df) == 'Group.2'] <- 'displayName'
names(df)[names(df) == 'Group.3'] <- 'round'

attach(df)
stat_df <- df[order(playerId, displayName, round),]
stat_df
```
...
write.csv(stat_df, "../../data/train_preprocessed_stats.csv", row.names=FALSE)
```
